...
video_input=r"C:\Users\a5566\OneDrive\Desktop\realsense_record\3.bag"
...
#----------------test parms------------------------
        self.skeleton =None
        self.skeleton_test_w=0
        self.skeleton_test_h=0
        self.frames=None
#-------------------------------------------
...
image_w, image_h = w, h
state.skeleton_test_w=w
state.skeleton_test_h=h
...
def draw_skeleton(intrinsics,joints,width, height):
    w, h = intrinsics.width, intrinsics.height
    batch = pyglet.graphics.Batch()
    print("START")
    for idx, landmark in enumerate(joints.pose_landmarks.landmark):
        #image_rows, image_cols, _ = 848,480,0
       ...
       # aligned_depth_frame.get_distance()
        ...
        #depth_pixel = [640,360] 
        #if landmark_px:
        #    color_pixel = [landmark_px[0], landmark_px[1]]   # Random pixel
        color_pixel = [landmark_px[0], landmark_px[1]]
        print("color_pixel",color_pixel)
        depth_point = rs.rs2_deproject_pixel_to_point(intrinsics, color_pixel, depth)
        
        print(depth_point)
        depth_point[0]=depth_point[0]*width
        depth_point[1]=depth_point[1]*height
        batch.add(2, gl.GL_LINES, None, ('v3f',  depth_point+[255,255,255]))
        batch.draw()
        
#        batch.add(2, gl.GL_LINES, None, ('v3f', [0, 0, 0] + [255,255,255]))
    

def find_skeleton_verts(points,joints,depth_frame,color_frame):
    """draw skeleton"""
    ...
    if(joints.pose_landmarks):
        ...
        for idx, landmark in enumerate(joints.pose_landmarks.landmark.NOSE):
            ...
            depth_scale = depth_sensor.get_depth_scale()
            # Map depth to color
            color_pixel = [landmark.x, landmark.y]   # Random pixel
            depth_point = rs.rs2_deproject_pixel_to_point(color_intrin, color_pixel, depth_scale)
            #color_point = rs.rs2_transform_point_to_point(depth_to_color_extrin, depth_point)
            #color_pixel = rs.rs2_project_point_to_pixel(color_intrin, color_point)
            skelton_verts.append(depth_point)
...
def on_draw():
    ...
    grid()
    gl.glPopMatrix()
    #if state.skeleton is not None and state.skeleton.pose_landmarks:
    #        if state.skeleton_test_w>0 and state.skeleton_test_h>0:
    #            draw_skeleton(color_intrinsics,state.skeleton,image_data.width, image_data.height)
...
#-------------------mediapipe start---------
    ...
    #cv2.imshow("depth", depth_colormap)
        ...
        #draw_skeleton(depth_intrinsics,depth_colormap,skeleton,depth_frame,color_image)
    if state.color:
    ...
    #verts= np.asarray(find_skeleton_verts(points,skeleton,depth_frame,other_frame))
